...
class AssetCompiler {
        ...
        try {
            ...
            for (int index = 0; index < filesToProcess.size(); index++) {
                ...
                futures << completionService.submit({ ->
                    ...
                    String fileSystemName = fileName.replace(AssetHelper.DIRECTIVE_FILE_SEPARATOR, File.separator)
                    ...
                    if (assetFile) {
                        ...
                        if (!(assetFile instanceof GenericAssetFile)) {
                            ...
                            String existingDigestFile = manifestProperties.getProperty("${fileName}${extension ? ('.' + extension) : ''}")
                            if (existingDigestFile && existingDigestFile == "${fileName}-${digestName}${extension ? ('.' + extension) : ''}") {
                                isUnchanged = true
                            }
                            if (fileName.indexOf(
                                    ".min") == -1 && contentType == 'application/javascript' && options.minifyJs && !isUnchanged && !isMinifyExcluded(
                                    assetFile.path)) {
                                ...
                                fileData = newFileData
                                    ...
                                    ".min") == -1 && contentType == 'text/css' && options.minifyCss && !isUnchanged && !isMinifyExcluded(
                                    assetFile.path)) {
                                def newFileData = fileData
                                try {
                                    // eventListener?.triggerEvent("StatusUpdate", "- Minifying File")
                        ...
                        if (!isUnchanged) {
                            ...
                            if (extension) {
                                if (options.enableDigests) {
                                    def digestedFile = new File(options.compileDir,
                                            "${fileSystemName}-${digestName}${extension ? ('.' + extension) : ''}")
                                    ...
                                    manifestProperties.setProperty("${fileName}${extension ? ('.' + extension) : ''}",
                                            "${fileName}-${digestName}${extension ? ('.' + extension) : ''}")
                                }
                                else {
                                    manifestProperties.setProperty("${fileName}${extension ? ('.' + extension) : ''}",
    ...
    private File initializeWorkspace() {
        // Check for existing Compiled Assets
        File assetDir = new File(options.compileDir)
        if (assetDir.exists()) {
