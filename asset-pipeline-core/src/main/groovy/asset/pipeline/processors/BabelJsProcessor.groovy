...
class BabelJsProcessor extends AbstractProcessor {
        ...
        if (!context) {
            synchronized ($LOCK) {
                if (!context) {
                    ...
                    context = Context.newBuilder().allowExperimentalOptions(true).allowHostAccess(
                            HostAccess.newBuilder().allowListAccess(true).allowMapAccess(true).allowArrayAccess(true).build()).build()
    ...
    String process(String input, AssetFile assetFile) {
        ...
        if (assetFile instanceof JsAssetFile) {
            if ((!newEcmascriptKeywordsFound && !AssetPipelineConfigHolder.config?.enableES6 && !AssetPipelineConfigHolder.config?."enable-es6") ||
					(newEcmascriptKeywordsFound && (AssetPipelineConfigHolder.config?.enableES6 == false || AssetPipelineConfigHolder.config?."enable-es6" == false))) {
                return input
            }
        }
        try {
